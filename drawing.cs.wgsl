...
@group(0) @binding(1) var inside_depth_tex: texture_2d<f32>;    // xyz=inside, w=depth
@group(0) @binding(2) var normal_material_tex: texture_2d<f32>; // xyz=normal, w=material
@group(0) @binding(3) var output_tex: texture_storage_2d<rgba32float, write>;
@group(0) @binding(4) var<storage, read_write> depth_buffer: array<atomic<u32>>;
...
fn cs_main(@builtin(global_invocation_id) global_id: vec3<u32>) {
	...
	// Bounds check
	if (target_x < 0 || target_x >= i32(screen_size.x) ||
	    target_y < 0 || target_y >= i32(screen_size.y)) {
		return;
	}

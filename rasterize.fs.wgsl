...
struct FragmentInput {
	...
	@location(0) inside: vec3<f32>,    // Anchor point (GPU interpolated)
	@location(1) normal: vec3<f32>,    // Surface direction (GPU interpolated)
	@location(2) depth: f32,           // Distance to surface (GPU interpolated)
	@location(3) material_id: f32,     // Material identifier
}

struct FragmentOutput {
	@location(0) inside_depth: vec4<f32>,   // xyz = inside position, w = depth
	@location(1) normal_material: vec4<f32>, // xyz = normal, w = material_id
}

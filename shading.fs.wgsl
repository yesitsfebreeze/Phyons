...
@group(0) @binding(1) var gbuffer_material: texture_2d<f32>;
@group(0) @binding(2) var gbuffer_distance: texture_2d<f32>;
@group(0) @binding(3) var depth_front: texture_depth_2d;
@group(0) @binding(4) var depth_back: texture_depth_2d;
...
fn fs_main(in: FragmentInput) -> @location(0) vec4<f32> {
	...
	if material_id < 0.5 {
		base_color = vec3<f32>(0.3, 0.3, 0.4);  // Inner layer - dark blue-gray
	} else {
		base_color = vec3<f32>(0.9, 0.85, 0.8);  // Outer layer - warm white
	}
